  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ teamName, seasonLabel, name, email, password }),
      });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-8">
      <div className="w-full max-w-sm card p-6">
        ...
        <p className="text-slate-500 mb-6">You&apos;ll be the team owner and can invite co-coaches with a join code.</p>
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            <label className="label" htmlFor="teamName">Team name</label>
            <input id="teamName" className="input" required value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="Sharks U8" />
          </div>
          <div>
            <label className="label" htmlFor="seasonLabel">Season label (optional)</label>
            <input id="seasonLabel" className="input" value={seasonLabel} onChange={(e) => setSeasonLabel(e.target.value)} placeholder="Fall 2026" />
          </div>
          <div>
